...
import { login, ok, QA_REF } from "./biochemistry-client.mjs";
...
const {db:admin,user}=await login();
const file="docs/biochemistry-import-manifest.json",prior=existsSync(file)?read(file):null;
const run={new_subjects:0,new_chapters:0,new_questions:0,new_options:0,new_answer_keys:0,new_tables:0,new_import_identities:0,unchanged_questions:0,uploads:0};
...
writeFileSync("docs/biochemistry-quarantine.json",JSON.stringify(quarantined,null,2)+"\n");
const manifest={batch,qa_project:QA_REF,authenticated_admin:user.id,approved_starting_sha:"5baafb97a22668bd7115dbf36636f4a5f7dda49a",preflight_sha256:hash(readFileSync("docs/biochemistry-preflight.json")),taxonomy,records:[],runs:prior?.runs||[]};
const progress=".local-qa/biochemistry-import-progress.json";
...
for(const q of quarantined)manifest.records.push({question_id:null,source_identity:sourceIdentity(q),source_key:q.source_key,source_document:q.source_document,source_sha256:q.source_sha256,bio:q.bio,source_sequence:q.source_sequence,status:"quarantine",manifest:"docs/biochemistry-quarantine.json",reasons:q.quarantine_reasons});
