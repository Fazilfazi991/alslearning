...
export function StudentShell({children,user}:{children:React.ReactNode;user:{name:string;email:string}}){
  ...
  return <div className="min-h-screen bg-surface">
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-[272px] flex-col border-r border-line bg-white p-5 lg:flex">
      ...
      <div className="mt-auto space-y-1"><Link href="/student/notifications" className="flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm font-semibold text-muted hover:bg-surface"><Bell size={19}/>Notifications</Link><Link href="/student/help" className="flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm font-semibold text-muted hover:bg-surface"><CircleHelp size={19}/>Help & Support</Link><div className="mt-4 flex items-center gap-3 rounded-xl border border-line p-3"><div className="grid h-10 w-10 place-items-center rounded-full bg-deep-blue text-xs font-bold text-white">{user.name.split(/\s+/).map(x=>x[0]).join("").slice(0,2).toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold">{user.name}</p><p className="truncate text-[10px] text-muted">{user.email}</p></div><ChevronDown size={15}/></div></div>
    </aside>
    ...
    {more&&<div className="fixed inset-0 z-50 bg-ink/35 lg:hidden" onClick={()=>setMore(false)}><div className="absolute inset-x-0 bottom-0 rounded-t-2xl bg-white p-5" onClick={e=>e.stopPropagation()}><div className="flex items-center justify-between"><h2 className="font-bold">More</h2><button onClick={()=>setMore(false)} className="grid h-10 w-10 place-items-center rounded-xl" aria-label="Close more menu"><X/></button></div><div className="mt-4 grid grid-cols-2 gap-3">{[{label:"Progress",href:"/student/progress",icon:TrendingUp},{label:"Certificates",href:"/student/certificates",icon:GraduationCap},{label:"Notifications",href:"/student/notifications",icon:Bell},{label:"Help",href:"/student/help",icon:CircleHelp},{label:"Profile",href:"/student/help",icon:UserRound}].map(({label,href,icon:Icon})=><Link key={label} href={href} onClick={()=>setMore(false)} className="flex min-h-16 items-center gap-3 rounded-xl border border-line p-3 text-sm font-bold"><Icon className="text-brand" size={19}/>{label}</Link>)}<LogoutButton className="flex min-h-16 w-full items-center gap-3 rounded-xl border border-line p-3 text-sm font-bold text-red-700"/></div></div></div>}
  </div>;
}
