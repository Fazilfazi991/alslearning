...
import {newQuestion,saveCoreQuestion,type CoreData,type Question} from "@/lib/core-repository";
...
function QuestionBankContent(){
  ...
  return <div className="min-w-0">
    ...
    {detail&&<Editor title="Edit question" close={()=>{if(!busy)setDetail(null);}}>{saveError&&<p role="alert" className="mb-3 text-red-800">{saveError}</p>}{detail.error?<div role="alert">{detail.error} <button className={retryClass} onClick={()=>void open(detail.id)}>Retry question</button></div>:detail.value&&academic?<QuestionForm key={detail.id} value={detail.value} data={academic} busy={busy} save={save}/>:<p role="status">Loading question details…</p>}</Editor>}
  </div>;
}
