  ...
  async function upload(file: File) {
    ...
    try {
      ...
      for (let partNumber = 1; partNumber <= count; partNumber++) {
        ...
        for (let attempt = 0; attempt < 3 && !response?.ok; attempt++) response = await fetch(url, { method: "PUT", body: file.slice(start, Math.min(file.size, start + session!.partSize)) });
