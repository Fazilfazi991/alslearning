...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!recording?.provider_video_id) return <section className="card mx-auto max-w-3xl p-6"><h1 className="text-xl font-bold">Recording unavailable</h1><p className="my-3 text-sm text-muted">This recording may no longer be available, or your course access may have expired. Contact ALS if you need help.</p><Link className="text-brand underline" href="/student/recorded-classes">Back to Recorded Classes</Link></section>;
  ...
  return <article className="mx-auto max-w-4xl space-y-5">
    ...
    <YouTubePlayer key={recording.id} videoId={recording.provider_video_id} title={recording.title}/>
