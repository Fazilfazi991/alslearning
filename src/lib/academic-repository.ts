...
const tableByKind = {
  ...
  topic: "topics",
  batch: "batches",
  video: "learning_content",
  material: "learning_content",
} as const;
function tableFor(kind: AcademicEntity["kind"]) {
  if (!(kind in tableByKind))
...
const questionTypeToDb: Record<QuestionType, string> = {
  ...
  "Image-based MCQ": "image_mcq",
  "Case-based": "case_based",
  "Match-the-following": "match_following",
};
const questionTypeFromDb = Object.fromEntries(
  Object.entries(questionTypeToDb).map(([a, b]) => [b, a]),
) as Record<string, QuestionType>;
const sourceToDb: Record<QuestionSource, string> = {
  Standard: "standard",
  "Previous exam": "previous_exam",
  Recalled: "recalled",
};
const sourceFromDb = Object.fromEntries(
  Object.entries(sourceToDb).map(([a, b]) => [b, a]),
) as Record<string, QuestionSource>;

function entity(
  row: Row,
  kind: AcademicEntity["kind"],
  parentId?: string,
): AcademicEntity {
  return {
    id: String(row.id),
...
export async function loadAcademicWorkspace(): Promise<AcademicWorkspace> {
  ...
  [
    ...
    content,
    questions,
  ].forEach((x) => assert(x.error));
  const entities: AcademicEntity[] = [
    ...(exams.data || []).map((x) => entity(x, "exam")),
    ...(programs.data || []).map((x) =>
      entity(x, "program", x.exam_id || undefined),
    ),
    ...(subjects.data || []).map((x) => entity(x, "subject")),
    ...(chapters.data || []).map((x) =>
      entity(x, "chapter", x.program_id || x.subject_id),
    ),
    ...(topics.data || []).map((x) =>
      entity(x, "topic", x.chapter_id || x.subject_id),
    ),
    ...(batches.data || []).map((x) =>
      entity({ ...x, display_order: 0 }, "batch", x.program_id),
    ),
    ...(content.data || []).map((x) =>
      entity(
        x,
        x.kind === "video" ? "video" : "material",
        x.topic_id || x.chapter_id || x.subject_id || x.program_id,
      ),
    ),
  ];
  const byId = new Map(entities.map((x) => [x.id, x]));
  const mappedQuestions: AcademicQuestion[] = (questions.data || []).map(
    (q) => {
      ...
      return {
        id: q.id,
        question: q.prompt,
        type: questionTypeFromDb[q.type] || "Single-answer MCQ",
        exam: byId.get(q.exam_id)?.name || "",
        program: byId.get(q.program_id)?.name,
        subject: byId.get(q.subject_id)?.name || "",
        chapter: byId.get(q.chapter_id)?.name,
        ...
        sourceType: sourceFromDb[q.source_type] || "Standard",
        sourceReference: q.source_reference || undefined,
        examYear: q.exam_year || undefined,
...
export async function saveAcademicEntity(
  item: AcademicEntity,
  all: AcademicEntity[],
) {
  const db = createClient(),
  ...
  if (item.kind === "topic") {
    const subjectId =
      parent?.kind === "subject"
        ? parent.id
        : parent?.kind === "chapter"
          ? all.find((x) => x.id === parent.parentId && x.kind === "subject")
              ?.id
          : undefined;
    if (!subjectId)
      throw new Error(
        "A topic must belong to a subject or a chapter with a subject.",
      );
    payload.subject_id = subjectId;
    payload.chapter_id = parent?.kind === "chapter" ? parent.id : null;
    payload.program_id = null;
  }
  if (item.kind === "batch") {
    payload = {
  ...
  if (item.kind === "video" || item.kind === "material") {
    payload = {
      ...
      title: item.name,
      description: item.description || null,
      kind: item.kind === "video" ? "video" : "note",
      status: dbStatus(item.status),
      display_order: item.order,
      external_url: "about:blank",
      program_id: parent?.kind === "program" ? parent.id : null,
      subject_id: parent?.kind === "subject" ? parent.id : null,
      chapter_id: parent?.kind === "chapter" ? parent.id : null,
      topic_id: parent?.kind === "topic" ? parent.id : null,
    };
  }
...
export async function persistEntityOrder(items: AcademicEntity[]) {
  await Promise.all(
    items
      .filter((x) => x.kind !== "batch")
      .map((item) =>
        createClient()
          .from(tableFor(item.kind))
          .update({ display_order: item.order })
          .eq("id", item.id)
          .then(({ error }) => assert(error)),
      ),
  );
}
...
export async function importQuestions(
  rows: AcademicQuestion[],
  entities: AcademicEntity[],
) {
  let imported = 0;
  for (const row of rows) {
    await saveQuestion(row, entities);
    imported++;
  }
  return imported;
}
...
export async function loadQuestionPage(
  ...
  if (source !== "All")
    query = query.eq("source_type", sourceToDb[source as QuestionSource]);
  if (type !== "All")
    query = query.eq("type", questionTypeToDb[type as QuestionType]);
  if (subjectId) query = query.eq("subject_id", subjectId);
  const { data, error, count } = await query;
  assert(error);
  const byId = new Map(entities.map((x) => [x.id, x]));
  const questions: AcademicQuestion[] = (data || []).map((q) => {
    const options = (q.question_options || []).sort(
      (a: Row, b: Row) => Number(a.display_order) - Number(b.display_order),
    );
    return {
  ...
  return { questions, count: count || 0, page, size };
}
